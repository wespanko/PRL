...
const NAV_ITEMS = [
  ...
  { id: "tutor",     label: "Live Tutor", icon: Eye            },
];
...
export default function Sidebar({ activeTab, setActiveTab, hasResults, profile, onSignOut }) {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-60 bg-white border-r border-slate-200 flex flex-col py-5 px-3 z-30">
      ...
      <nav className="flex-1 space-y-1">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon
                className={`h-5 w-5 shrink-0 ${active ? "text-indigo-600" : ""}`}
                strokeWidth={2.25}
              />
      ...
      {/* Profile dock */}
      <div className="relative pt-3 border-t border-slate-200" ref={menuRef}>
        {menuOpen && (
          <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-slate-200 rounded-lg shadow-lg p-2 z-10">
            ...
            <button
              className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-slate-500 hover:bg-slate-50"
            ...
            <button
              className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-rose-600 hover:bg-rose-500/10"
