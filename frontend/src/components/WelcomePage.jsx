        ...
        return (
          <div key={s.id} className="flex-1 flex items-center gap-2">
            <div className={`h-1.5 rounded-full flex-1 transition-colors duration-500
              ${active ? "bg-indigo-600" : done ? "bg-blue-300" : "bg-slate-200"}`} />
            <span className={`text-xs font-semibold whitespace-nowrap
              ${active ? "text-indigo-600" : done ? "text-indigo-600" : "text-slate-500"}`}>
              {s.label}
            </span>
          </div>
        );
...
function ChoiceCard({ active, icon: Icon, label, body, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full text-left rounded-xl p-5 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]
        ${active
          ? "bg-indigo-50 border-2 border-indigo-600 shadow-sm shadow-indigo-100"
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {active && <CheckCircle2 className="h-4 w-4 text-indigo-600" strokeWidth={2.5} />}
          </div>
...
export default function WelcomePage({ onSignIn }) {
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-900 font-sans antialiased px-6 py-10 md:px-10 md:py-16">
      <div className="max-w-2xl mx-auto">
        ...
        {step === "hero" && (
          <>
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-center text-slate-900 mb-5 leading-tight">
              Your portfolio,<br/>
              <span className="text-indigo-600">analyzed like a quant.</span>
            </h1>
            ...
            <button
              ...
              className="w-full mb-10 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold text-lg py-5 flex items-center justify-center gap-2 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] active:scale-[0.98] shadow-md shadow-indigo-200"
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-12">
              {TRUST_BADGES.map((b) => {
                const Icon = b.icon;
                return (
                  <div key={b.label} className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-1">
                      <Icon className="h-4 w-4 text-indigo-600" strokeWidth={2.5} />
            ...
            <div className="text-xs text-slate-500 leading-relaxed text-center max-w-lg mx-auto">
              <strong className="text-slate-500">Educational tool · Not financial advice.</strong> Panko computes
            ...
            <form onSubmit={handleNameStep} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  autoFocus
                  maxLength={60}
                  className="w-full bg-white border border-slate-200 rounded-lg px-5 py-4 text-base font-medium text-slate-900 placeholder:text-slate-500 outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-100 transition-colors"
                />
              </div>
              <div>
              ...
              {error && (
                <div className="bg-rose-50 border border-rose-200 rounded-lg px-4 py-3 text-sm font-medium text-rose-700">
                  {error}
                </div>
              )}
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="px-6 py-3.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-500 font-bold transition-colors"
                >
                  Back
                </button>
                <button
        ...
        {step === "risk" && (
          <>
            ...
            <div className="mt-8 text-xs text-slate-500 text-center leading-relaxed">
              <strong className="text-slate-500">Reminder:</strong> Panko is educational. It is not your financial advisor. Risk style helps tune the engine — it does not constitute advice.
            </div>
          </>
        )}
        ...
        {step === "experience" && (
          <>
            ...
            <label className="mt-6 flex items-start gap-3 bg-slate-50 border border-slate-200 rounded-lg p-4 cursor-pointer">
              <input
                type="checkbox"
            ...
            {error && (
              <div className="mt-3 bg-rose-50 border border-rose-200 rounded-lg px-4 py-3 text-sm font-medium text-rose-700">
                {error}
              </div>
            )}
